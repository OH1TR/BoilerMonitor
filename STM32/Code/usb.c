		...
		uint16_t *len, void (**complete)(usbd_device *usbd_dev, struct usb_setup_data *req))
{
	...
	unsigned int i;
	console_SendString("Creq:");
        console_SendHex(req->bRequest,4);
	console_SendString("\r\n");
	...
	case USB_CDC_REQ_SET_CONTROL_LINE_STATE: {
		...
		console_SendString("SET_LINE_CODING:");
                console_SendHex(req->wValue,2);
		console_SendString("\r\n");
		return 1;
		}
	case USB_CDC_REQ_SET_LINE_CODING:
		if (*len >= sizeof(struct usb_cdc_line_coding))
		{
			...
			console_SendString("SET_LINE_CODING:");
			for(i=0;i<sizeof(struct usb_cdc_line_coding);i++)
				console_SendHex((unsigned int)buf[i],2);
			console_SendHex(req->wIndex,2);
			console_SendString("\r\n");
...
static void cdcacm_data_rx_cb(usbd_device *usbd_dev, uint8_t ep)
{
	...
	if (len) 
	{
		glue_handle_rx_data_from_usb(buf,len,ep);
	}
}
...
static void cdcacm_set_config(usbd_device *usbd_dev, uint16_t wValue)
{
	...
	usbd_ep_setup(usbd_dev, 0x86, USB_ENDPOINT_ATTR_INTERRUPT, 16, NULL);
...
{
	usbd_ep_nak_set(usbd_dev,addr,nak);
}
